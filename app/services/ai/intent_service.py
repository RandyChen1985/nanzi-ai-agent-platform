...
fact_kind 只能描述事实类型，例如 business_metric、machine_load、file_count、
...
_DATA_QUERY_STRONG_SIGNALS = [
    ...
    "销售额", "订单量", "转化率", "库存", "成本", "收入", "完成率", "销量",
    "经营指标", "业务指标", "支付金额", "客单价", "利润",
...
class IntentResponse(BaseModel):
    ...
    fact_kind: str = Field(
        default="unknown",
        description="The kind of fact or result involved in the request",
    )
    freshness_requirement: str = Field(
        default="unknown",
        description="How fresh the evidence must be for this request",
    )
