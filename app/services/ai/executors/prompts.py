...
"""
    ...
    GLOBAL_GUARDRAILS = (
        ...
        "10.1 查询性能：Schema 中的 partition_fields 是分区裁剪字段，涉及时间/范围查询时必须优先生成对应过滤条件；"
        "index_fields 是可利用的索引字段，等值过滤、范围过滤和 JOIN 应优先使用这些字段。"
        "禁止 SELECT *、无 WHERE 的明细全表扫描和无界大结果集；必须设置合理 LIMIT/聚合范围。"
        "若用户问题无法确定分区范围且查询可能扫描大表，先追问时间范围，不得冒险生成高风险 SQL。\n"
        ...
        "16. 维度与下钻：Schema 中字段若带 `dimension_role`（time/geo/category/identifier），表示该字段是分析维度，"
        "适合用于 GROUP BY 分组统计；`dimension_role=none` 或无该标记的字段不得作为分组维度。"
        "当用户要求「按XX维度看/按XX拆开/下钻到XX」时，优先在 Schema 中查找 `dimension_role != none` 的字段；"
        "若多个字段 `hierarchy_group` 相同，它们构成一条下钻路径，按 `hierarchy_order` 从小到大=从粗到细，"
        "下钻时沿 hierarchy_order 递增方向切换到更细粒度的同组字段做 GROUP BY。"
        "无法确定下钻目标时，先列出可用维度与层级供用户选择，不得臆造维度字段。"
    )
