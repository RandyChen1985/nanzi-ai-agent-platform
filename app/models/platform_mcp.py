...
class McpOAuthClient(Base):
    ...
    allowed_agent_ids = Column(JSON, nullable=True, comment="Client 可访问的智能体白名单，NULL 表示不额外限制")
    allowed_knowledge_base_ids = Column(JSON, nullable=True, comment="Client 可访问的知识库白名单，NULL 表示不额外限制")
    allowed_metadata_dataset_ids = Column(JSON, nullable=True, comment="Client 可访问的元数据集白名单，NULL 表示不额外限制")
