...
```text
.
├── app/                  # 后端核心代码 (FastAPI 异步架构)
│   ├── api/              # API 路由层 (Portal 运营端与 V1 客户端 REST/SSE 接口)
│   ├── core/             # 系统核心基础设施 (全局配置、数据库引擎、安全加密、生命周期)
│   ├── models/           # SQLAlchemy 2.x 数据库 ORM 实体模型
│   ├── schemas/          # Pydantic 契约与 DTO 数据校验模型
│   ├── services/         # 核心业务服务层 (鉴权、模型管理、ChatBI、RAG、MCP 调度)
│   │   └── ai/           # 🤖 AI 编排中心 (AgentScope 运行器、OpenClaw 执行器、意图分发器)
│   └── utils/            # 通用工具库 (加解密、向量算子、数据转换与辅助工具)
├── frontend/             # 前端工程 (Vue 3 + Vite + TypeScript + Tailwind CSS)
│   └── src/
│       ├── api/          # 前端接口请求层与后端契约映射
│       ├── views/        # 业务页面 (控制台、智能体编排、ChatBI、知识库、技能中心等)
│       ├── components/   # 通用 UI 组件与聊天流式卡片
│       ├── composables/  # Vue 组合式状态逻辑 (SSE 流式会话、权限管理)
│       └── router/       # 动态路由与菜单权限控制
├── .agent/               # Agent 自动化开发技能与工作流体系 (opsx、dev-skills 等)
├── architech/            # 顶层架构设计、契约规范与系统提示词 (Prompts) 管理中心
├── data/                 # 平台持久化运行数据 (工作区、沙箱运行时、技能资产、文件上传)
├── db-prod/              # MySQL 历史版本迁移与 SQL 升级脚本 (V0-VNN)
├── db-prod-pg/           # PostgreSQL 基线与幂等版本迁移体系 (V0-VNN)
├── docker/               # 容器化镜像构建方案与 docker-compose 一键启动编排
...
NanZi AI 智能体平台是一个完全开源且持续迭代演进的项目。如果本项目在学习、工作或业务落地中对您有所帮助，欢迎请作者喝杯咖啡 ☕！

您的每一份支持，都是激励项目持续打磨架构、扩展核心功能与维护高质量文档的巨大动力。衷心感谢所有支持开源的小伙伴！

<div align="center">
  <img src="docs/images/donate.png" alt="微信赞赏支持" width="220" /><br/>
  <sub>微信赞赏支持（作者 NanZi）</sub>
</div>
