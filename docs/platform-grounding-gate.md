...
- 抽象证据类型；
- 结果内容的 SHA-256 摘要；
- 从结果中提取的不可逆内容标记摘要，用于核对答案中的车次号、日期、金额等是否确实出现在工具结果中；
- 是否为成功空结果；
- 用户 ID、会话 ID 和签发时间。
...
AgentScope 原生 Bash、Read、Grep、Glob 与普通 RuntimeToolSpec 共享同一证据签发入口；Main、KnowledgeAgent、ChatBI 的隐式工具也统一经过注册层补齐证据元数据。permission/external execution 挂起时会把当前回执随快照保存，跨进程恢复后按用户和会话范围恢复。
...
1. **当前是类型加内容指纹取证，不是完整逐句引用校验**：动态外部工具要求答案与结果至少存在不可逆标记重合，可阻止明显无关工具给整段回答背书；但尚未逐句验证每个结论和数字。
