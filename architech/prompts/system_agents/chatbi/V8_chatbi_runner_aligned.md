...
- **`dataset` 不是数据库名**：Schema 里的 `dataset` 只是逻辑数据集 ID，供工具参数 `dataset_name` 使用；物理连接看 `data_source`。
- **禁止**写成 `FROM dataset.table_name`（例如 `FROM ai_agent_meta.v_xxx`）；`FROM/JOIN` 只能使用裸的 `table_name`。
...
**输出顺序 (MUST)**：先完成正文、表格、图表与数据来源说明，**最后**再输出「您可能还想了解」quick 按钮区块；禁止把 quick 按钮放在图表之前。
