...
在南孜智能体平台中，大模型的多轮交互会面临滚雪球式的 Token 膨胀。本文件深入评估了当前平台中所有智能体（Assistant、DataQuery、Knowledge）对历史消息的处理现状，并提出了针对性的历史裁剪优化方案。
...
大模型在多轮对话中可能输出长 Markdown 表格与图表配置，针对历史表格的压缩策略设计如下：
1. **深度保护最近一轮（Last Assistant）**：用户在多轮交互中最常见的是基于上一轮生成的数据做翻译、追问或求和计算。因此最近一轮的 Assistant 回复深度放宽门槛（最多保留 100 行数据，约 1.5k~3k Token），完整覆盖绝大部分业务全量表格，保证用户紧随其后追问时数据 100% 完整，绝不发生误截断。
2. **提高常规表格容纳门槛**：非最近一轮的更早历史表格，默认保留 30 行数据明细（足以覆盖绝大部分日常对比表、排行榜、清单等业务表格）；且超出行数达到 10 行以上才执行折叠，彻底杜绝“为了省略少量行数而负收益截断”的问题。
3. **独立引用块提示防模型回显**：折叠提示采用独立的块引用 `> [系统说明：更早历史表格数据较长，此处已折叠后续 X 行明细]`，不再伪装成 Markdown 表格单元格，防止大模型误认为数据行而在后续回答中照抄复述。
4. **图表 JSON 过滤**：如果大模型在上一轮中输出了大段 ```chart JSON``` 格式的 ECharts 图表参数，直接在历史中清空。

* **收益评估**：既完全保护了最近一轮及中短表格的数据完整性，又能对更早轮次超长巨型表格实现 Token 保护。
