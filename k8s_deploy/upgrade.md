...
> [!IMPORTANT]
> **行为变更：K8s 沙箱默认与平台同命名空间，并自动共享平台数据卷（V156/V157、V57/V58 迁移）**
>
...
>    无需再手工填写任何 PVC 名称；
> 3. 数据库迁移 `V156`（MySQL）/ `V57`（PG）会把仍为历史默认值的命名空间配置对齐为 `nanzi-ai-agent`；
...
>
> **候选版本如何识别**：脚本读取节点容器运行时（`k3s ctr` / `crictl` / `ctr -n k8s.io`）中全部 `nanzi-ai-agent` Tag，解析出完整镜像仓库与 Tag（支持 `registry:5000/repo/name:tag` 带端口形态），再按**版本号逐段数值比较**：
>
> - 推荐值 = 节点上**严格高于当前运行版本的最高数字版本**，不会再像旧版那样把任意非当前 Tag（含更低版本、`latest` 等纯别名）都当作「候选新版本」；
> - 列表按版本升序展示，并分别标注「当前运行中 / 候选新版本 / 旧版本，不建议回退 / 纯别名 Tag，不参与版本比较」；
> - 节点上不存在更高版本时，默认值回填当前运行 Tag（走 `rollout restart` 重新加载同名镜像），并提示先导入新 tar；
> - 若显式选择的 Tag 低于集群当前运行版本，会额外要求二次确认（版本回退保护）。
>
