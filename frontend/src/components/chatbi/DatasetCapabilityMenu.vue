              ...
              <div class="relative flex-shrink-0 flex items-center gap-0.5 mt-0.5">
                ...
                >
                  ...
                  <div
                    ...
                    class="absolute right-0 top-full z-30 mt-1 w-52 overflow-hidden rounded-xl border border-gray-200 bg-white p-1 shadow-xl dark:border-gray-700 dark:bg-gray-900 space-y-0.5"
                  >
                    <button
                      type="button"
                ...
                <button
                  type="button"
                  class="flex items-center justify-center w-6 h-6 rounded-lg transition-all duration-200 active:scale-90"
                  :class="isCardCollapsed(group)
                    ? 'text-blue-600 bg-blue-50 dark:bg-blue-900/30 opacity-100'
                    : 'text-gray-300 dark:text-gray-600 opacity-0 group-hover/card:opacity-100 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800'"
                  :title="isCardCollapsed(group) ? '展开卡片内容' : '折叠收起卡片'"
                  :aria-expanded="!isCardCollapsed(group)"
                  @click.stop="toggleCollapseCard($event, group)"
                >
                  <svg
                    class="w-3.5 h-3.5 transform transition-transform duration-300"
                    :class="{ 'rotate-180': !isCardCollapsed(group) }"
...
const collapsedCardKeys = ref<Record<string, boolean>>({});

const isCardCollapsed = (group: DatasetCapabilityGroup): boolean => {
  const id = groupKey(group);
  return Boolean(collapsedCardKeys.value[id]);
};

const toggleCollapseCard = (event: MouseEvent, group: DatasetCapabilityGroup) => {
  event.stopPropagation();
  const id = groupKey(group);
  collapsedCardKeys.value[id] = !collapsedCardKeys.value[id];
  try {
    localStorage.setItem('portal_collapsed_cards', JSON.stringify(collapsedCardKeys.value));
  } catch (e) {}
  savePortalPrefs();
};
...
const loadPortalPrefs = async () => {
  try {
    try {
      const local = localStorage.getItem('portal_collapsed_cards');
      if (local) collapsedCardKeys.value = JSON.parse(local);
    } catch (e) {}
    ...
    if (data.collapsed_card_keys && typeof data.collapsed_card_keys === 'object') {
      collapsedCardKeys.value = { ...collapsedCardKeys.value, ...data.collapsed_card_keys };
    }
    ...
    try {
      await axios.put("/api/portal/portal-prefs", {
        ...
        expanded_group_ids: Object.entries(expandedGroups.value)
          .filter(([, v]) => v).map(([k]) => k),
        collapsed_card_keys: collapsedCardKeys.value,
